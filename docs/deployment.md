...
Resource profile: 256 MiB hard memory, 128 MiB reservation, 384 MiB memory+swap, 0.25 CPU, PID 64, tmpfs `/tmp` 32 MiB. No published ports. The encryption secret is mounted read-only for UID/GID `10001:10001` with mode `0400`, so the non-root process can read it without broadening access.
